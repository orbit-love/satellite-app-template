...
import { fetchIdentityUrls } from "./member-helpers";
...
export async function upsertMember(member, included) {
  ...
  const { twitter_url, linkedin_url } = fetchIdentityUrls(member, included);

  return prisma.member.upsert({
    where: { email },
    update: { name, avatar_url, twitter_url, linkedin_url },
    create: { email, name, avatar_url, twitter_url, linkedin_url },
  });
}
